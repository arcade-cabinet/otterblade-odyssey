...
function loadChaptersWithColors() {
    try {
        ...
        const unmappedChapters = chapters.filter((ch) => !biomes.some((b) => b.chapterIds.includes(ch.id)));
        if (unmappedChapters.length > 0) {
            console.warn(`[constants] ${unmappedChapters.length} chapters have no biome mapping: ` +
                unmappedChapters.map((ch) => ch.name).join(', '));
        }
        return chapters.map((ch, idx) => {
            const colors = biomeColors[idx];
            if (!colors) {
                console.warn(`[constants] Missing biome colors for chapter ${idx} (${ch.name}), using fallback.`);
            }
            return {
                ...ch,
...
export const CHAPTERS = loadChaptersWithColors();
