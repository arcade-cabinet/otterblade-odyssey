...
import { usePhysics2D } from './Physics2D';
import { useStore } from './store';
import { hash1 } from './utils';
function Platform({ id, position, size, kind = 'plain' }) {
    ...
    const [x, y] = position;
    const [w, h] = size;
    return (React.createElement("mesh", { position: [x, y, 0], receiveShadow: true },
...
function ProceduralPlatforms() {
    ...
    return (React.createElement(React.Fragment, null, platforms.map((p) => (React.createElement(Platform, { key: p.id, id: p.id, position: p.position, size: p.size })))));
}
function SimpleEnvironment() {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("fog", { attach: "fog", args: [fogColor, 20, 80] }),
        React.createElement("color", { attach: "background", args: [skyColor] })));
}
export function Level() {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(ParallaxBackgroundSystem, { biomeIndex: biomeIndex, playerX: playerX }),
        React.createElement(SimpleEnvironment, null),
        React.createElement(ProceduralPlatforms, null),
        React.createElement("ambientLight", { intensity: 0.4 }),
        React.createElement("pointLight", { position: [playerX + 10, 15, 5], intensity: 0.8, castShadow: true })));
}
