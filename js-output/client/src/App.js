...
import CinematicPlayer from '@/components/hud/CinematicPlayer';
...
function AppContent() {
    const [introWatched, setIntroWatched] = useState(null);
    ...
    if (introWatched === null) {
        return (React.createElement("div", { className: "w-full h-screen bg-black flex items-center justify-center" },
            React.createElement("div", { className: "text-amber-200/60 animate-pulse" }, "Loading...")));
    }
    return (React.createElement("div", { className: "relative w-full h-screen overflow-hidden bg-black", style: { touchAction: 'none' } },
        React.createElement(Game, null),
        React.createElement(PostFX, null),
        React.createElement(DamageFlash, null),
        React.createElement(HUD, null),
        React.createElement(ChapterPlate, null),
        React.createElement(StartMenu, null),
        React.createElement(GameOverMenu, null),
        React.createElement(TouchControls, null),
        currentCinematic && (React.createElement(CinematicPlayer, { type: currentCinematic, onComplete: handleCinematicComplete, skippable: true }))));
}
function App() {
    ...
    return (React.createElement(ThemeProvider, { theme: otterbladeTheme },
        React.createElement(CssBaseline, null),
        React.createElement(QueryClientProvider, { client: queryClient }, isAssetReviewRoute ? (React.createElement(Suspense, { fallback: React.createElement("div", { className: "w-full h-screen bg-black flex items-center justify-center" },
                React.createElement("div", { className: "text-amber-200/60 animate-pulse" }, "Loading Asset Review...")) },
            React.createElement(AssetReview, null))) : (React.createElement(AppContent, null)))));
}
export default App;
