...
const Carousel = React.forwardRef(({ orientation = 'horizontal', opts, setApi, plugins, className, children, ...props }, ref) => {
    ...
    return (React.createElement(CarouselContext.Provider, { value: {
        ...
        React.createElement("div", { ref: ref, onKeyDownCapture: handleKeyDown, className: cn('relative', className), role: "region", "aria-roledescription": "carousel", ...props }, children)));
});
Carousel.displayName = 'Carousel';
const CarouselContent = React.forwardRef(({ className, ...props }, ref) => {
    const { carouselRef, orientation } = useCarousel();
    return (React.createElement("div", { ref: carouselRef, className: "overflow-hidden" },
        React.createElement("div", { ref: ref, className: cn('flex', orientation === 'horizontal' ? '-ml-4' : '-mt-4 flex-col', className), ...props })));
});
CarouselContent.displayName = 'CarouselContent';
const CarouselItem = React.forwardRef(({ className, ...props }, ref) => {
    const { orientation } = useCarousel();
    return (React.createElement("div", { ref: ref, role: "group", "aria-roledescription": "slide", className: cn('min-w-0 shrink-0 grow-0 basis-full', orientation === 'horizontal' ? 'pl-4' : 'pt-4', className), ...props }));
});
CarouselItem.displayName = 'CarouselItem';
const CarouselPrevious = React.forwardRef(({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
    const { orientation, scrollPrev, canScrollPrev } = useCarousel();
    return (React.createElement(Button, { ref: ref, variant: variant, size: size, className: cn('absolute  h-8 w-8 rounded-full', orientation === 'horizontal'
