...
function Calendar({ className, classNames, showOutsideDays = true, captionLayout = 'label', buttonVariant = 'ghost', formatters, components, ...props }) {
    ...
    return (React.createElement(DayPicker, { showOutsideDays: showOutsideDays, className: cn('bg-background group/calendar p-3 [--cell-size:2rem] [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent', String.raw `rtl:**:[.rdp-button\_next>svg]:rotate-180`, String.raw `rtl:**:[.rdp-button\_previous>svg]:rotate-180`, className), captionLayout: captionLayout, formatters: {
            ...
            Root: ({ className, rootRef, ...props }) => {
                return React.createElement("div", { "data-slot": "calendar", ref: rootRef, className: cn(className), ...props });
            },
            Chevron: ({ className, orientation, ...props }) => {
                if (orientation === 'left') {
                    return React.createElement(ChevronLeftIcon, { className: cn('size-4', className), ...props });
                }
                if (orientation === 'right') {
                    return React.createElement(ChevronRightIcon, { className: cn('size-4', className), ...props });
                }
                return React.createElement(ChevronDownIcon, { className: cn('size-4', className), ...props });
            },
...
function CalendarDayButton({ className, day, modifiers, ...props }) {
    ...
    return (React.createElement(Button, { ref: ref, variant: "ghost", size: "icon", "data-day": day.date.toLocaleDateString(), "data-selected-single": modifiers.selected &&
